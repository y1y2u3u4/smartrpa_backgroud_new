        ...
        try {
            if (!this.element.leixing) {
                ...
                try {
                    ...
                    // 等待并点击用户按钮
                    await page.waitForSelector('#user.channel', { timeout: 5000 });
                    await page.evaluate(() => {
                        const userButton = document.querySelector('#user.channel');
                        if (!userButton) {
                            throw new Error('未找到用户按钮');
                        }
                        userButton.click();
                        console.log('已点击用户按钮');
                    });
                    ...
                    // 等待并点击第一个用户结果
                    await page.waitForSelector('.user-item-box', { timeout: 5000 });

                    // 获取browser对象
                    const browser = this.browser;
                    if (!browser) {
                        throw new Error('browser对象未定义');
                    }

                    // 设置新页面监听和超时
                    const newPagePromise = new Promise((resolve, reject) => {
                        const timeout = setTimeout(() => {
                            reject(new Error('等待新页面超时'));
                        }, 10000);
                    ...
                    while (retryCount < maxRetries && !newPage) {
                        try {
                            // 点击用户结果
                            await page.evaluate(async () => {
                                const firstUserResult = document.querySelector('.user-item-box');
                                if (!firstUserResult) {
                                    throw new Error('未找到用户结果');
                                }
                                const userLink = firstUserResult.querySelector('.user-info').parentElement;
                                userLink.click();
                            });
                            console.log(`已点击用户结果，等待新页面打开... (尝试 ${retryCount + 1}/${maxRetries})`);

                            // 等待新页面打开
                            newPage = await newPagePromise;
                            break;
                        } catch (error) {
                            console.log(`尝试 ${retryCount + 1} 失败: ${error.message}`);
                            retryCount++;
                            if (retryCount === maxRetries) {
                                throw new Error(`新页面创建失败，已尝试 ${maxRetries} 次`);
                            }
                            // 等待一下再重试
                            await new Promise(resolve => setTimeout(resolve, 2000));
                        }
                    ...
                    if (!newPage) {
                        throw new Error('新页面创建失败');
                    }

                    // 切换到新页面
                    page = newPage;
                    console.log('已切换到新页面:', await page.url());

                    // 等待页面加载和内容准备
                    try {
                        // 先等待页面URL变化
                        await page.waitForFunction(
                            url => window.location.href.includes('user/profile'),
                            { timeout: 10000 },
                            'user/profile'
                        );
                        ...
                        console.log('等待页面加载超时:', error.message);
                        // 即使超时也继续执行
                    }
                    ...
                    this.data = await page.evaluate(async () => {
                        ...
                        function extractNoteInfo(element) {
                            ...
                            try {
                                // 提取笔记ID
                                const hiddenLink = element.querySelector('a[href^="/explore/"]');
                                info.noteId = hiddenLink ? hiddenLink.getAttribute('href').split('/').pop() : 'N/A';

                                // 提取标题
                                const titleElement = element.querySelector('.title span[data-v-0cdd7be0]');
                                info.title = titleElement ? titleElement.textContent.trim() : 'N/A';

                                // 提取作者信息
                                const authorElement = element.querySelector('.author-wrapper .author');
                                if (authorElement) {
                                    info.authorName = authorElement.querySelector('.name').textContent.trim();
                                    info.authorAvatar = authorElement.querySelector('img.author-avatar').src;
                                    info.authorLink = authorElement.href;
                                    // 从作者链接中提取用户ID
                                    const authorId = authorElement.href.match(/profile\/([^?]+)/);
                                    info.authorId = authorId ? authorId[1] : 'N/A';
                                }

                                // 提取封面图片
                                const coverElement = element.querySelector('.cover.ld.mask img');
                                info.coverImage = coverElement ? coverElement.src : 'N/A';

                                // 提取笔记链接
                                const linkElement = element.querySelector('.cover.ld.mask');
                                ...
                                // 提取置顶状态
                                const topElement = element.querySelector('.top-tag-area .top-wrapper');
                                info.isTop = topElement ? topElement.textContent.trim() === '置顶' : false;

                                // 提取图片尺寸信息
                                info.imageWidth = element.getAttribute('data-width');
                                info.imageHeight = element.getAttribute('data-height');
                                info.imageIndex = element.getAttribute('data-index');
                            } catch (error) {
                        ...
                        while (scrollAttempts < maxScrollAttempts) {
                            // 获取当前可见的笔记卡片
                            const noteElements = document.querySelectorAll('section[data-v-14f91c23].note-item');
