                ...
                try {
                    await page.evaluate(async (cliclValue) => {
                        ...
                        while (retryCount < maxRetries) {
                            ...
                            retryCount++;
                            console.log(`重试查找搜索框: 第${retryCount}次`);
                        }
...
export class OutputTask extends Task {
    ...
    async execute(page) {
        ...
        if (this.element.leixing === '自定义1') {
            ...
            await page.waitForFunction(() => {
                return (
                    document.querySelector('tr.table-row-module_row_JSSv0') !== null ||
                    document.querySelector('.text-view-module_textView_NY5Q0.text-view-module_headline-h2_\\+teY9') !== null
                );
            }, { timeout: 30000 }).catch(error => {
                console.log('等待数据超时:', error);
            });
            this.data = await page.evaluate(() => {
                const noContentElement = document.querySelector('.text-view-module_textView_NY5Q0.text-view-module_headline-h2_\\+teY9');
                if (noContentElement && noContentElement.textContent.includes('未找到任何内容')) {
                    return [];
                }
