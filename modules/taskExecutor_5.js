        ...
        if (this.element.leixing === '自定义1') {
            ...
            this.data = await page.evaluate(() => {
                ...
                function extractProductInfo(element) {
                    ...
                    // 提取标题
                    const titleElement = element.querySelector('h2 a.a-link-normal, a[data-type="productTitle"]');
                    info.title = titleElement ? titleElement.textContent.trim() : 'N/A';

                    // 提取评分
                    const ratingElement = element.querySelector('.a-icon-star-small, .a-icon-star-mini');
                    info.rating = ratingElement ? ratingElement.getAttribute('aria-label') : 'N/A';

                    // 提取评论数
                    const reviewCountElement = element.querySelector('.s-underline-text, .a-size-small.a-color-tertiary');
                    ...
                    // 提取图片URL
                    const imageElement = element.querySelector('img[data-image-latency="s-product-image"]');
                    info.imageUrl = imageElement ? imageElement.src : 'N/A';

                    return info;
                }

                // 获取所有产品卡片
                const productElements = document.querySelectorAll('div[data-component-type="s-search-result"], div[data-asin]');
                const products = Array.from(productElements).map(extractProductInfo);
