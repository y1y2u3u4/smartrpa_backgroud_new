    ...
    async execute(page) {
        ...
        try {
            ...
            try {
                ...
                // 设置产品状态的函数
                async function setProductStatus(page) {
                    return await retryOperation(async () => {
                        console.log('开始设置产品状态...');
                        const selector = '.ui-search_input .el-select--small input[placeholder="请选择"]';
                        ...
                        if (!selected) {
                            throw new Error('未找到"在售"选项');
                        }

                        await new Promise(resolve => setTimeout(resolve, 2000));
                        
                        const buttonClicked = await page.evaluate(() => {
                            const buttons = Array.from(document.querySelectorAll('.el-button--primary'));
                            const searchButton = buttons.find(button => {
                                const text = button.textContent.trim();
                                return text.includes('查询') || button.querySelector('.el-icon-search');
                            });
                ...
                async function goToPage(page, pageNumber) {
                    return await retryOperation(async () => {
                        ...
                        if (actualPage !== pageNumber) {
                            throw new Error(`页码跳转失败，期望页码: ${pageNumber}，实际页码: ${actualPage}`);
                        }
                    });
                }

                // 获取页面数据的函数
                async function getPageData(page, currentPage) {
                    return await retryOperation(async () => {
                        const pageData = await page.evaluate((currentPage) => {
                            ...
                            return Array.from(rows).map(row => {
                                ...
                                return {
                                    ...
                                    number: getCellContent(3),
                                    category: getCellContent(4),
                                    sku: getCellContent(5),
                                    image_url: getImageUrl(),
                                    title_en: getCellContent(7),
                                    title_cn: getCellContent(8),
                                    dimensions: getCellContent(9),
                                    weight: getCellContent(10),
                                    price: getCellContent(11),
                                    stock: getCellContent(12),
                                    warehouse: getCellContent(13),
                                    turnover_days: getCellContent(16),
                                    package_size: getCellContent(19),
                                    expiry_date: getCellContent(20),
                                    target_countries: getCellContent(21),
                                    status: getCellContent(24),
                                    stock_status: getCellContent(25),
                                    listing_status: getCellContent(26),
                                    update_time: getCellContent(28)
                                };
                            });
                        }, currentPage);

                        if (!pageData || pageData.length === 0) {
                            throw new Error(`第${currentPage}页没有获取到数据`);
                        }
                ...
                const pageList = [4434, 4435, 4436, 4437, 4572, 4573, 6167, 6222, 6223];
                // 遍历所有页码
                for (const currentPage of pageList) {
                    try {
                        // 跳转到指定页面
                        await goToPage(page, currentPage);
                        
                        // 获取当前页数据
                        const pageData = await getPageData(page, currentPage);
                ...
                if (failedPages.length > 0) {
                    console.log('失败的页码:', failedPages);
                }

                if (totalRecords === 0) {
                    console.log('整个过程没有提取到任何数据');
                } else {
                    console.log(`数据采集完成，共处理${totalRecords}条数据，失败${failedPages.length}页`);
                }
