...
export class OutputTask extends Task {
    ...
    async execute(page) {
        ...
        if (this.element.leixing === '自定义1') {
            ...
            try {
                // 等待页面加载完成
                await page.waitForSelector('#noteContainer', { timeout: 10000 });
                ...
                this.data = await page.evaluate(() => {
                    ...
                    try {
                        ...
                        // 获取标题
                        const title = getTextContent('#detail-title');
                        
                        // 获取描述内容
                        const desc = getTextContent('#detail-desc');
                        ...
                        // 获取点赞数和收藏数
                        const likes = getTextContent('.like-wrapper .count') || '0';
                        const collects = getTextContent('.collect-wrapper .count') || '0';
                        
                        // 获取图片链接
                        const images = [];
                        const imageEls = document.querySelectorAll('.note-slider-img');
                        ...
                        // 获取发布时间
                        const publishTime = getTextContent('.date') || '';

                        // 验证必要字段
                        if (!author || !title) {
                            throw new Error('未能获取到作者或标题信息');
                        }
                        ...
                        console.error('页面数据提取错误:', error.message);
                        throw error;
                    }
                });

                console.log('笔记详情信息:', JSON.stringify(this.data, null, 2));
                ...
                console.error('小红书笔记信息采集失败:', error.message);
                throw new Error(`笔记信息采集失败: ${error.message}`);
            }
        }
