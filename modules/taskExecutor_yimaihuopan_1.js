        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (!selected) {
                        console.log('未找到"在售"选项，继续执行后续操作');
                        return;
                    }
                    ...
                    if (!buttonClicked) {
                        console.log('未找到查询按钮，继续执行后续操作');
                        return;
                    }
                    
                    // 等待查询结果加载
                    await new Promise(resolve => setTimeout(resolve, 5000));
                    console.log('产品状态已设置为在售并执行查询');
                } catch (error) {
                    console.error('设置产品状态或执行查询时出错：', error);
